...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def ReadRanges(self):
        ...
        # lm_pos  = self.M 
        # robot_x_pos = self.xsk[0,0]
        # robot_y_pos = self.xsk[1,0]
        # lm_dis = np.zeros((len(lm_pos),1))
        
        # for idx, pos in enumerate(lm_pos):
        #     dis = ((pos[0,0]-robot_x_pos)**2 + (pos[1,0]-robot_y_pos)**2)**0.5 
        #     lm_dis[idx,0] = dis

        # noise = np.random.normal(0,0.3,lm_dis.shape)
