...
class MCLocalization(ParticleFilter):
    ...
    def Prediction(self, u, Q):
        ...
        #     updated_particle = self.MotionModel(self.particles[i], u, Q)
        #     updated_particles.append(updated_particle)

        # self.particles = updated_particles  # Update particles in-place

        # return self.particles

        output_particles = self.particles.copy()
        for idx ,particle in enumerate(output_particles):
            noise = np.random.multivariate_normal([0,0],Q).reshape(2,1)
            particle = self.MotionModel(particle,u,noise) 
            output_particles[idx] = particle
        self.particles = output_particles
