...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        """ Motion model used to simulate the robot motion. Computes the current robot state :math:`x_k` 
        given the previous robot state :math:`x_{k-1}` and the input :math:`u_k`:
    ...
    def ReadEncoders(self):
        ...
        **To be completed by the student**.
        ...
        :return zsk,Rsk: :math:`zk=[n_L~n_R]^T` 
        observation vector containing number of pulses read from the left and right wheel encoders. 

        :math:`R_{s_k}=diag(\\sigma_L^2,\\sigma_R^2)` covariance matrix (for noise = incr and dec likewise) of the read pulses.
        """
        ...
        #wheelbase =distance bw left and right wheel
        # wheelbase/2 = distance from the robot's CENTER
        # - bcz left wheel will move in opposite direction 
        # meanwhile in right wheel movement its alr +      
