...
class MapFeature:
    ...
    * **The storage representation:** which is the representation used to store the feature in the map within the state vector.
    ...
    def hf(self, xk,map_i): 
        ...
        The method computes the expected observations for the observed features contained within the features observation vector.
        To do it, it iterates over each feature observation calling the method for its corresponding associated feature
        to compute the expected observation, collecting all them in the returned vector.
    ...
    def Jhfjx(self, xk, Fj):  # Jacobian wrt x of the observation function for feature observation i
        ...
        # NxB= Pose3D(NxB)
        # J = self.J_s2o(self.M[Fj].boxplus(NxB.ominus())) @ NxFj.J_1boxplus(NxB.ominus()) @ NxB.J_ominus()
    ...
    def Jgx(self, xk, BxFj):  # Jacobian wrt xk of the inverse sensor model for a single feature observation
        ...
        NxB = Pose3D(xk[0:xBpose_dim,0].reshape((xBpose_dim,1)))
        # a=self.o2s(BxFj).J_1boxplus(NxB)
        # J = np.block([CartesianFeature.J_1boxplus(self.o2s(BxFj), NxB), np.zeros((xF_dim, xB_dim-xF_dim))])
        # J = np.block([self.o2s(BxFj).J_1boxplus(NxB), np.zeros((xF_dim, xB_dim-xF_dim))])
...
class Cartesian2DStoredPolarObservedMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        ...
        """
        # TODO: To be implemented by the student
        zk, Rk = self.robot.ReadPolarFeature()
        if len(zk) != 0:
            self.featureData = True
        return zk, Rk
