...
class GL(HF):
    ...
    def LocalizationLoop(self, p0, usk):
        ...
        # same as Localization.py for DR
        ...
        #     self.pk.plot_histogram() # Plotting the belief
    ...
    def Localize(self, pxk_1, uk, zk):
        """
        Solves a localization iteration calling, 
        successively to the :meth:`HF.Prediction` first, followed by the :meth:`HF.Update`.
        ...
        # prediction (total probability) using uk
        # pk_hat = self.Prediction(pxk_1, uk)
        # self.Prediction(pxk_1, uk)

        # ---------update-------------------
       
        # pk_hat = predicted position of the robot after a motion update

        # # start pk_hat - uniform belief for all cells
        # pk_hat = Histogram2D(self.p0.num_bins_x, self.p0.num_bins_y, self.p0.x_range, self.p0.y_range)

        # # initialize = equal probablities = 1
        # pk_hat.histogram = np.ones(pk_hat.num_bins_x * pk_hat.num_bins_y)

        # # normalize= sum all P = 1 = valid probability distribution.
